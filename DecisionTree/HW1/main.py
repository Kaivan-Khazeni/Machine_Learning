...
if __name__ == '__main__':
    ...
    df = pd.read_csv('Car_Data/train.csv', names=attributes)
    df_test = pd.read_csv('Car_Data/test.csv', names=attributes)
    print(get_error_of_trees(df,df_test))
    ...
    for attr in temp_fix_unknown.columns:
        if bank_df_train[attr].value_counts().keys()[0] == "unknown":
            temp_fix_unknown.loc[(temp_fix_unknown[attr] == "unknown"), attr] = temp_fix_unknown[attr].value_counts().keys()[1]
        else:
